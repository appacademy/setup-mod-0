...
3. Follow the instructions on the installer to complete the download and
   installation.
4. To open the VS Code editor, open the Command Palette (`Cmd+Shift+P`) and type
   "shell command".
5. Click on `Shell Command: Install 'code' command in PATH command` - this will
...
7. Determine what your shell startup file is by running `echo $SHELL` in the
   ...
   - If your terminal output doesn't display any of the outputs above, ask for
     help.
8. Run `code "$HOME/.${SHELL#*/bin/}rc"` to open your shell startup file in your
   VS Code editor to edit the file.
9. At the end of the file, add the following lines of code:
...
11. Open a brand new terminal.
12. Run `nvm install node` to install Node.js
13. Then type the next command in your terminal: `nvm -v` and press Enter.
...
14. When installation is complete, open your terminal and type: `which node`
    and press Enter.
...
15. Run the following command in the terminal to install some Node.js packages
